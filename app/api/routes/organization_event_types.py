...
def list_organization_event_types(
    ...
    Returns:
        List[EventTypeResponse]: List of organization event types.

    Raises:
        HTTPException: If user is not a member of the organization.
    """
    logger.info(
        "org_event_types_list_started",
    ...
    if not is_org_member(db, current_user, organization_id):
        logger.warning(
            "org_event_types_list_forbidden",
            organization_id=organization_id,
            user_id=current_user.id
        )
        raise HTTPException(
    ...
    query = db.query(EventType).options(
        ...
        EventType.project_id.is_(None)  # Org-level only
    )
...
def create_organization_event_type(
    ...
    if not can_manage_organization(db, current_user, organization_id):
        logger.warning(
            "org_event_type_create_forbidden",
            organization_id=organization_id,
            user_id=current_user.id
        )
        raise HTTPException(
...
def get_organization_event_type(
    ...
    event_type = db.query(EventType).options(
        ...
        EventType.organization_id == organization_id,
        EventType.project_id.is_(None)
    ).first()

    if not event_type:
        logger.warning("org_event_type_not_found", event_type_id=event_type_id)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Event type not found"
        )

    logger.info("org_event_type_retrieved", event_type_id=event_type_id)
    return event_type
...
def update_organization_event_type(
    ...
    logger.info("org_event_type_updated", event_type_id=event_type_id)
    return event_type
...
def delete_organization_event_type(
    ...
    logger.info("org_event_type_deleted", event_type_id=event_type_id)
