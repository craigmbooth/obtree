...
class EventTypeField(Base):
    """Event type field model - defines fields for event types.

    Similar to ProjectAccessionField and ProjectPlantField, this defines
    the schema for event fields with type-specific validation rules.
    """

    __tablename__ = "event_type_fields"

    id = Column(GUID, primary_key=True, default=uuid_lib.uuid4, index=True)
    event_type_id = Column(GUID, ForeignKey("event_types.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    # Relationships
    event_type = relationship("EventType", back_populates="fields")
    creator = relationship("User")
    field_values = relationship("EventFieldValue", back_populates="field", cascade="all, delete-orphan")
